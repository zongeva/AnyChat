...
//import com.kpz.AnyChat.Network.Http_SkillUse;
import com.kpz.AnyChat.Others.ChatMsgUtil;
import com.kpz.AnyChat.Others.DialogUtil;
//import com.kpz.AnyChat.Others.GlobalVariable;
...
public class ChatActivity extends AppCompatActivity {
    ...
    long othersideid, fwd_id;
    EditText inputwindow;
    ImageButton btnOpenPopup;
    EditText txtpopup;
    List<ChatMsg> chatMsgList = new ArrayList<>();
    boolean burnChat = false, stunt = false, blackout = false, tornado = false;
    Context context;
    long touchTime;
    long txt_fwd_id;
    ConversationHistoryArrayAdapter mAdapter;
    String pitch;
    ...
    long lastMsgID;
    String eat_char = "";
    String next_msg_eat_char = "";
    Boolean eat_char_bool = false;
    Boolean next_msg_eat;
    Dialog dialog;
    int counter;
    ArrayList<Long> handler = new ArrayList<Long>();
    public static long lastClickTime = 0;
    public static final long DOUBLE_CLICK_TIME_DELTA = 100;
    int clickcounter = 0;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.e("Freeze ", freeze_time + "");
        chatService.setMsgReads(new RequestCallBack() {
            @Override
            public void handleSuccess(Object o, Object o2, Object o3) {
                Byte bite = 2;
                Log.e("UCC Log", "code: 1602014 ReadCount1: " + chatService.getUnreadMsgCountByType(bite) + "");
        ...
        final ImageView text_msg_state = (ImageView) findViewById(R.id.text_msg_state);
        ...
        * */
        ...
        SharedPreferences sharedPref = ChatActivity.this.getPreferences(ChatActivity.this.MODE_PRIVATE);
        String nme = getResources().getString(R.string.next_msg_eat);
        if (nme.equals("true")) {
            next_msg_eat = true;
            Log.e("UCC Log", "code: 1603011 next_msg_eat set to TRUE from shared preference.");
        } else {
            next_msg_eat = false;
            Log.e("UCC Log", "code: 1603012 next_msg_eat set to FALSE from shared preference.");
        }
        ...
        final String chat_title_id = Objects.toString(othersideid);
        TextView swipe_notify = (TextView) findViewById(R.id.swipe_notify);
        ...
        final Http_GetToken gt = new Http_GetToken(ChatActivity.this, 10, url_get_token, ids, dum, deviceId, "4", "", "");
        if (dum2.getText().toString().equals("")) {
            ...
            Http_GetToken cb = new Http_GetToken(ChatActivity.this, 9, url_join_group, ids, dum2, deviceId, "", "", "");
            cb.execute();
        }
        /*
        * Check if one exist in group
        * */
        if (ChatMsgApi.isGroup(othersideid)) {
            final Http_GetToken check_in_group = new Http_GetToken(ChatActivity.this, 12, url_get_token, ids, dum, deviceId, "4", "", "");
            check_in_group.execute();
        }
        ...
        * */
                    ...
                    RequestHelper.getMessages(TargetID, Math.max(lastMsgID, 0), 30, flag, SendUserID, new RequestCallBack<Long, List<ChatMsg>, Void>() {
                        ...
                        public void handleSuccess(Long aLong, List<ChatMsg> list, Void aVoid) {
                            ...
                            for (int i = list.size() - 1; i >= 0; i--) {
                                ...
                                ah_listView.setAdapter(new ConversationHistoryArrayAdapter(getApplicationContext(), chatMsgList, ChatActivity.this));
                                ah_listView.setSelection(30);
                                swipeView.setRefreshing(false);
                            }
        ...
        if (ChatMsgApi.isGroup(othersideid)) {
            ...
            TextView temp = (TextView) findViewById(R.id.tempq);
...
//        ah_listView.setTranscriptMode(ListView.TRANSCRIPT_MODE_NORMAL);
//        ah_listView.setStackFromBottom(true);
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (count > 0) {// 文本框输入内容
                    ...
                    if (ChatMsgApi.isGroup(othersideid) && substring.equals("@")) {
                        useEdit = inputwindow;
                        hideSoftKeyboard();
                        Intent intent = new Intent((ChatActivity) ChatActivity.this, SelectGroupMemberTag.class);
        ...
        inputwindow.setOnKeyListener(new View.OnKeyListener() {
            @Override
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                ...
                if (keyCode == KeyEvent.KEYCODE_DEL && event.getAction() == KeyEvent.ACTION_DOWN) { //当为删除键并且是按下动作时执行
                    int selectionStart = inputwindow.getSelectionStart();
                    int lastPosition = 0;
                    String tempString = inputwindow.getText().subSequence(0, selectionStart).toString();
                    for (Map.Entry<Long, String> entry : atUserMap.entrySet()) {
                        if ((lastPosition = tempString.indexOf(entry.getValue(), lastPosition)) != -1) {
                            if (selectionStart != 0 && selectionStart >= lastPosition && selectionStart <= (lastPosition + entry.getValue().length())) {
                                inputwindow.getText().delete(lastPosition, selectionStart);
        ...
        ClientManager.getDefault().getChatService().getMessages(othersideid, msgID, count, flags, new ResultCallBack<Long, List<ChatMsg>, Void>() {
            @Override
            public void onSuccess(Long aLong, final List<ChatMsg> chatMsgs, Void aVoid) {
                Log.e("UCC Log", "code " + Utils.osType + "1601005 ChatMsg List size:" + chatMsgs.size());

                if (chatMsgs.size() != 0) {
                    chatMsgList = chatMsgs;
                    ah_listView.setAdapter(new ConversationHistoryArrayAdapter(ChatActivity.this, chatMsgList, ChatActivity.this));


                    if (positions != 0) {
                        ah_listView.setAdapter(new ConversationHistoryArrayAdapter(ChatActivity.this, chatMsgList, ChatActivity.this));
                        ah_listView.setSelection(positions);

                        Log.e("Index", positions + " This part of code did run");
                    } else {
                        ah_listView.setSelection(chatMsgList.size() - 1);
                    }
                        ...
                        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                            ah_listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
                                ...
                                public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
                                    ...
                                    final String recall = getApplicationContext().getString(R.string.vim_recall_msg);
...
//                                    if (RequestHelper.isMyself(chatMsgList.get(position).getFromID())) {
                                    ...
                                    switch (chatMsgList.get(position).getMsgType()) {
                                        case ChatMsgApi.TYPE_TEXT:
                                            if (!isMe || isTaskMsg || isDelayMsg || isReceiptMsg) {
                                                items = new CharSequence[]{copy, reply, forward, /* transferToTask,*/ delete};
                                            } else if (isPrivacyMsg) {
//                            } else if (isDeleteMsg || isFlashMsg || isPrivacyMsg) {
                                                items = new CharSequence[]{delete, reply, moreOptions};
                                            } else {
                                                items = new CharSequence[]{copy, forward, delete, recall};
                                            }
                                            break;
                                        case ChatMsgApi.TYPE_FILE:
                                            if (!isMe || isPrivacyMsg) {
                                                items = new CharSequence[]{forward, reply, export, delete};
                                            } else {
                                                items = new CharSequence[]{forward, export, delete, recall};
                                            }
                                            break;
                                        ...
                                        case ChatMsgApi.TYPE_AUDIO:
                                            if (!isMe) {
                                                items = new CharSequence[]{forward, export, reply, delete};
                                            } else {
                                                items = new CharSequence[]{forward, export, delete, recall};
                                            }
                                            break;
                                        ...
                                        case ChatMsgApi.TYPE_CUSTOM_DYNAMIC:
                                            if (!isMe || isPrivacyMsg || isTaskMsg || isDelayMsg || isReceiptMsg) {
                                                items = new CharSequence[]{forward, reply, delete, moreOptions};
                                            } else {
                                        ...
                                        case ChatMsgApi.TYPE_TEMPL:
                                            if (!isMe || isPrivacyMsg || isTaskMsg || isDelayMsg || isReceiptMsg) {
                                                items = new CharSequence[]{forward, reply, delete};
                                            } else {
                                                items = new CharSequence[]{forward, delete, recall};
                                            }
                                            break;
                                        ...
                                        public void onSelection(MaterialDialog materialDialog, View view, int i, CharSequence charSequence) {
                                            ...
                                            if (s.equals(forward)) {
                                                switch (chatMsgList.get(position).getMsgType()) {
                                                    ...
                                                    case ChatMsgApi.TYPE_MINI_VIDEO:
                                                    case ChatMsgApi.TYPE_AUDIO:
                                                        Intent intent = new Intent(ChatActivity.this, SelectChatActivity.class);
                                                        ...
                                                        startActivityForResult(intent, SELECT_FOWARD_TARGET);
                                                        break;
...
//                                                itemListener.onItemOperation(OptionBean.TYPE_OPTION_MSG_FORWARD, chatMsgList.get(position));
                                                ...
                                                if (cur_time > (msg_time + 120000)) {
                                                    Toast.makeText(ChatActivity.this, "You can only recall a message that sent within 2 minutes.", Toast.LENGTH_SHORT).show();
                                                } else {
                                                ...
                                                itemListener.onItemOperation(OptionBean.TYPE_OPTION_MSG_MORE, chatMsgList.get(position));
                                                ...
                                                if (ChatMsgApi.TYPE_IMAGE == chatMsgList.get(position).getMsgType()) {
                                                    ...
                                                    Toast.makeText(ChatActivity.this, "Image Exported To " + destPath, Toast.LENGTH_SHORT).show();
                                                    ...
                                                    RequestHelper.decryptFile(key, downPath, destPath);
                                                    Toast.makeText(ChatActivity.this, "Audio Exported To " + destPath, Toast.LENGTH_SHORT).show();
                                                    ...
                                                    RequestHelper.decryptFile(key, downPath, destPath);
                                                    Toast.makeText(ChatActivity.this, "MiniVideo Exported To " + destPath, Toast.LENGTH_SHORT).show();
                                                    ...
                                                    RequestHelper.decryptFile(key, downPath, destPath);
                                                    Toast.makeText(ChatActivity.this, "File Exported To " + destPath, Toast.LENGTH_SHORT).show();

                                                }
                                            } else {
                                                Toast.makeText(ChatActivity.this, "Message Type Not Support Yet", Toast.LENGTH_SHORT).show();

                                            }
                                        }
                                    };
                                    DialogUtil.buildOperateDialog(ChatActivity.this, items, itemOperateCallBack).show();
        ...
        chatService.observeReceiveListener(othersideid, new ReceiveMsgListener() {
            ...
            public void receive(final ChatMsg chatMsg) {
                ...
                //test
                if (chatMsg.getMsgID() == 91861) {
                    List<Long> listMsgID = new ArrayList<Long>();
                    listMsgID.add(chatMsg.getMsgID());
                    chatService.deleteMsgByID(othersideid, listMsgID, new ResultCallBack() {
                        @Override
                        public void onSuccess(Object o, Object o2, Object o3) {
                            Log.e("delete", "91861 success");
                        }

                        @Override
                        public void onError(int i, String s) {
                            Log.e("delete", "91861 failed");
                        }
                    });
                }
                //end test

                Log.e("UCC Log", "code " + Utils.osType + "1601006 Success to receive message from listener (receive)" + chatMsg.getBody());

                //Retrieve new in and out message
                chatMsgList.add(chatMsg);
                ah_listView.setAdapter(new ConversationHistoryArrayAdapter(getApplicationContext(), chatMsgList, ChatActivity.this));
                //ah_listView.setSelection(chatMsgList.size() - 1);


                if (chatMsgList.get(position).getMsgType() == ChatMsgApi.TYPE_WEAK_HINT) {
                    final MsgTip msgTip = (MsgTip) chatMsgList.get(position);
                    if (msgTip.getOprType() == 5) {
                        Log.e("Delete System Notify", "Success");
                ...
                //Refresh list when receive any Revoke message
                if (ChatMsgApi.TYPE_REVOKE == chatMsg.getMsgType()) {
                    ah_listView.setAdapter(new ConversationHistoryArrayAdapter(getApplicationContext(), chatMsgList, ChatActivity.this));
                }

                // Check if any effect apply on everyone
                //check burn chat
...
//                    Toast.makeText(getApplicationContext(), "Burn Chat !", Toast.LENGTH_SHORT).show();
                ...
                Log.e("Related Receive CA", chatMsg.getRelatedUsers() + "");
...
//                                    Toast.makeText(getApplicationContext(), "Someone Open The Light For You", Toast.LENGTH_SHORT).show();
            ...
            public void update(ChatMsg chatMsg) {
                ...
                Log.e("UCC Log", "code " + Utils.osType + "1601007 Success to receive message from listener (update)");
...
//                                    chatMsg.setPreDefined(pitch);
                                ...
                                adapter.add(new MaterialSimpleListItem.Builder(ChatActivity.this)
                                        .content("Robot")
        ...
        down.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ah_listView.smoothScrollToPosition(chatMsgList.size() - 1);
                //ah_listView.setSelection(chatMsgList.size() - 1);
        ...
        ah_listView.setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                if (ah_listView.getLastVisiblePosition() == chatMsgList.size() - 1) {
                    down.setVisibility(View.INVISIBLE);
                    //hide button if at bottom
                } else if (ah_listView.getLastVisiblePosition() != chatMsgList.size() - 1) {
                    down.setVisibility(View.VISIBLE);

                    //show down button to directly go to latest message
                }
            }
        });
...
//            Toast.makeText(getApplicationContext(), "Blackout", Toast.LENGTH_SHORT).show();
    ...
    public void go_eat_func(View view) {
        if (!eat_char_bool) {
            ...
            eat_char_bool = false;
            Toast.makeText(ChatActivity.this, "Alphabet Eater disabled.", Toast.LENGTH_SHORT).show();
        }
    }
...
//
    ...
    public void enable_tornado(View view) {
        if (tornado == true) {
            ...
            inputwindow.setHint("Ab");
            popupWindow.dismiss();
//            Toast.makeText(getApplicationContext(), "Tornado  Enabled", Toast.LENGTH_SHORT).show();
        } else {
            txtpopup.setBackground(getDrawable(R.drawable.layout_bg));
            inputwindow.setBackground(getDrawable(R.drawable.layout_bg));
            tornado = true;
...
//                                    Toast.makeText(getApplicationContext(), "Image Upload Successful", Toast.LENGTH_SHORT).show();
        ...
        //end of camera activity
            ...
            if (resultCode == RESULT_OK) {
                ...
                time = 1000;
                final ChatMsg chatMsg = builder.createMiniVideoMsg(othersideid, flag, location, 200, 200, time);
...
//                                    Toast.makeText(getApplicationContext(), "Video Upload Successful", Toast.LENGTH_SHORT).show();
        ...
        //Image Selected
        else if (requestCode == SELECT_CAPTURED_IMAGE) {
...
//                                Toast.makeText(getApplicationContext(), "Image Upload Successful", Toast.LENGTH_SHORT).show();
            ...
            if (resultCode == RESULT_OK) {
                ...
                ChatMsgBuilder builder = new ChatMsgBuilder(othersideid);
                final ChatMsg chatMsg = builder.createFileMsg(path);
                if (burnChat == true) {
...
//                                    Toast.makeText(getApplicationContext(), "File Upload Successful", Toast.LENGTH_SHORT).show();
        ...
        } else if (requestCode == MINI_VIDEO_REQUEST_CODE) {

            Bundle bundle = getIntent().getExtras();
            String file = bundle.getString("text");//获得拍摄的短视频保存地址
            Log.e("Mini Video Path", file);

        } else if (requestCode == SELECT_GROUP_MEMBER) {
            ...
            btn.performClick();
        } else if (requestCode == SELECT_GROUP_MEMBERS) {
            hideSoftKeyboard();
            ImageView back_btn = (ImageView) findViewById(R.id.back_btn);
            handler = (ArrayList<Long>) data.getSerializableExtra("data");
            Log.e("Set RelatedUser : ", handler + "");
            ...
            for (int i = 0; i < handler.size(); i++) {
                ...
                Utils.setNickname(ChatActivity.this, url, handler.get(i) + "", nameholder);
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (inputwindow.isFocused()) {
                ...
                inputwindow.getGlobalVisibleRect(outRect);
                if (outRect.contains((int) event.getRawX(), (int) event.getRawY())) {
                    inputwindow.clearFocus();
                    //Wont Dismiss at this point
                    // Hide keyboard
//                    InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
//                    imm.hideSoftInputFromWindow(v.getWindowToken(), 0);
//                    inputwindow.setVisibility(GONE);
//                    ImageView img = (ImageView)findViewById(R.id.back_btn);
//                    img.setVisibility(GONE);
    ...
    public int redirect(int msgindex) {
        int msgIndex = msgindex;
        Toast.makeText(ChatActivity.this, "Index : " + msgindex, Toast.LENGTH_SHORT).show();
        return msgIndex;
    }

    public void scroll(ListView list, int msgindex) {
...
//        relatedUsers.clear();
//        Log.e("Text Related Clear : ",relatedUsers+"");
        ...
        * */
            ...
            if (message.getText().length() != 0) {
                ...
                if (!chatMsg.getPreDefined().equals("") || burnChat == true) {
                    ...
                    new MaterialDialog.Builder(ChatActivity.this)
                            .title("Remaining " + dum2.getText().toString())
                            .content("By Using Special Effect, You'll spent 1 energy")
                            .positiveText("Agree")
                            .negativeText("Cancel")
                            .onPositive(new MaterialDialog.SingleButtonCallback() {
                                @Override
